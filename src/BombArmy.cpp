...
BombArmy::BombArmy(float x, float y) :
    Army("play/bombs.png", x, y, 20, 0, 80, 15, 2, 0) {
...
void BombArmy::Update(float deltaTime) {
    ...
    if (Target) {
        ...
        int xx = static_cast<int>(floor(Target->Position.x / PlayScene::BlockSize));
        int yy = static_cast<int>(floor(Target->Position.y / PlayScene::BlockSize));
