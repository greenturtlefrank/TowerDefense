...
void Defense::Update(float deltaTime) {
    ...
    if (!Target) {
        ...
        for (auto& it : scene->ArmyGroup->GetObjects()) {
            ...
            if (InShootingRange(it->Position) && it) {
                Target = dynamic_cast<Army*>(it);
                if (Target->id >= 6 || Target->Hide == true)
                {
                    Target = nullptr;
                    continue;
                }
...
void Defense::Draw() const {
    ...
    if (HP < MAXHP) {
        ...
        al_draw_line(Position.x - 30, Position.y - 30, Position.x - 30 + 60 * (HP / MAXHP), Position.y - 30, al_map_rgb(200, 0, 0), 5);
    }
