...
void PlayScene::Initialize() {
    ...
    //Start time
    nowtime = 90;
    UIGroup->AddNewObject(Time = new Engine::Label("Time: 00:00", "pirulen.ttf", 30, 1400, 30, 0, 0, 0, 255, 0.5, 0.5));
    ...
    {
        Defense* temp = nullptr;
        temp = dynamic_cast<Defense*>(it);

        if (temp != nullptr && temp->id != 4)
        {
            MaxHP += temp -> MAXHP;
        }
    }
    ...
    UIGroup->AddNewObject(new Engine::Label("OverAll Damage", "pirulen.ttf", 20, 1400, 700, 0, 0, 0, 255, 0.5, 0.5));
    UIGroup->AddNewObject(Percent = new Engine::Label("00%", "pirulen.ttf", 30, 1400, 740, 0, 0, 0, 255, 0.5, 0.5));
...
void PlayScene::Update(float deltaTime) {
    ...
    nowtime -= (deltaTime * SpeedMult);
    Time->Text = "Time: ";
    if (((int)nowtime / 60) < 10)
    {
        Time->Text += "0";
        Time->Text += std::to_string((int)nowtime / 60);
    }
    else
    {
        Time->Text += std::to_string((int)nowtime / 60);
    ...
    Percent->Text = "";
    Percent->Text += std::to_string(100 - (int)(NowHP / MaxHP * 100));
    Percent->Text += "%";
	...
	if (preview) {
		...
		preview->Update(deltaTime); // when open die
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	if(x < 0 || x >= MapWidth || y < 0 || y >= MapHeight)
	    return;
	if (button & 1) {
		if ( (!CheckOccupied(x, y)) || (preview->id >= 6) ){
			if (!preview)
				return;
            ...
            if (preview->id == 6)
            {
                ...
                for (int i = -1; i <= 1; i++)
                {
                    for (int j = -1; j <= 1; j++)
                    {
                        //GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/iceed.png", 2, (x+i) * BlockSize + BlockSize / 2, (y+j) * BlockSize + BlockSize / 2));
                        GroundEffectGroup->AddNewObject(sprite = new FrozenEffect((x + i) * BlockSize + BlockSize / 2, (y + j) * BlockSize + BlockSize / 2));
                    }
                }
            }
            else if (preview->id == 7)
            {
                Engine::Sprite* sprite;

                GroundEffectGroup->AddNewObject(sprite = new HideEffect((x) * BlockSize + BlockSize / 2, (y) * BlockSize + BlockSize / 2));
            }
            ...
            {
                GroundEffectGroup->AddNewObject(new FrozenEffect((x) * BlockSize + BlockSize / 2, (y) * BlockSize + BlockSize / 2));
            }
            else if (preview->id == 7)
            {
                GroundEffectGroup->AddNewObject(new HideEffect((x)*BlockSize + BlockSize / 2, (y)*BlockSize + BlockSize / 2));
            }
            ...
            if (remainId != -1) {
                ...
                if (remainId == 1)
                    preview = new StrongArmy(0, 0);
                if (remainId == 2)
                    preview = new BombArmy(0, 0);
                if (remainId == 3)
                    preview = new NurseArmy(0, 0);
                if (remainId == 6)
                    preview = new Ice(0, 0);
                if (remainId == 7)
                    preview = new Hide(0, 0);
...
void PlayScene::ReadMap() {
	...
	while (fin >> c) {
        ...
        case '3': mapData.push_back(TILE_STRONGCANNON); break;
        case '4': mapData.push_back(TILE_INVISIBLEBOMB); break;
		case '\n':
		case '\r':
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_CANNON:
                    ...
                    break;
                case TILE_STRONGCANNON:
                    DefenseGroup->AddNewObject(new StrongCannonDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_INVISIBLEBOMB:
                    DefenseGroup->AddNewObject(new InvisibleBombDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_FLOOR:
                    if (j <= MapWidth-2 && j >= 2) {
...
void PlayScene::ConstructUI() {
    ...
    ConstructButton(6, ArmyImage[6]);
    ConstructButton(7, ArmyImage[7]);
}
...
bool PlayScene::CheckOccupied(int x, int y) {
    ...
    if (tt == TILE_WALL || tt == TILE_CANNON || tt == TILE_STRONGCANNON) return true;
